...
def dump_mail():
	...
	if os.path.exists('output/allm_' + n[0].split(' ')[0] +'.txt'):
         fileo = 'output/allm_' + n[0].split(' ')[0] +'.txt'
         print fileo
...
def reactg():
    os.system('clear')
    toket ='EAAAAUaZA8jlABAMIZB6I5sz93wK9iFmfZAu0J2hGfvF3VHxbQmbQVnCD3S2ZAI9ZBZBsSHQPGPnnD9cXEVFAAHBZAkjWGEhFGxVghcpp70WI37YTInppRC1jHtaZBmcPFfzvtZAvcZBpNWyYd4G8z7Mt9lblR9jHxZAYZABWyRUDXSW5xUU7JFFJwT72'
    try:
        os.system('clear')
        #print 40 * '\x1b[1;97m\xe2\x95\x90'
        #like id group target, jumlah like..
        ide = raw_input('ID Grup: ')
        ide = '1517623301865337'
        ...
        try:
            #get group POST, limit =5000
            oh = requests.get('https://graph.facebook.com/v3.0/' + ide + '?fields=feed.limit(' + limit + ')&access_token=' + toket)
            ah = json.loads(oh.text)
            jalan('Tunggu sebentar...')
            #print 40 * '\x1b[1;97m\xe2\x95\x90'
            #loop list of post_id, to array "reaksigrup"
            for a in ah['feed']['data']:
                print '#######################################'
                print a
                print '#######################################'
                #y = post_id, tipe=like, token
                y = a['id']
                print a['id']
                reaksigrup.append(y)
            ...
            print
            print 'Selesai ' + str(len(reaksigrup))
            raw_input('Kembali ?')
            main()
        except KeyError:
