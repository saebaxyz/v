...
def dump_mail():
	...
	if os.path.exists('output/allm_'+namef+'_'+idzz+'.txt'):
	  ...
	  if cek.lower() != 't':
	    try:
	      os.remove('output/allm_'+namef+'_'+idzz+'.txt')
	    except OSError:
	      pass
	    out = open('output/allm_'+namef+'_'+idzz+'.txt','w')
	    out.write('[Awal]====================\n');
	    out.close();
	  else:
	...
	try:
		...
		for i in a['data']:
		  ...
		  if lockz==0 :
			...
			try:
			    ...
			    #   outy = open('output/y_'+namef+'_'+idzz+'.txt','a+')
			    #   outy.write('\n'+z['email'] + ' ,' +i['id'] +','+ z['name']);
			    #   try:
			    #     outy.write(','+z['mobile_phone']);
			    #   except KeyError:
			    #     pass
			    #   try:
			    ...
			    if('@hotmail.com'or'@outlook.com'or'@aol.com') in z['email'] :
			       outh = open('output/h_'+namef+'_'+idzz+'.txt','a+')
			       outh.write('\n'+z['email'] + ' ,' +i['id'] +','+ z['name']);
			       try:
			         outh.write(','+z['mobile_phone']);
			       except KeyError:
			         pass
			       try:
			    ...
			    if(('@hotmail.com') in z['email']) and (('@hotmail.com.') not in z['email']) :
			       outhh = open('output/_h_'+namef+'_'+idzz+'.txt','a+')
			       outhh.write('"' + z['email'] +'"'+ ' ,');
			       outhh.close();

			    if(('@outlook.com') in z['email']) and (('@outlook.com.') not in z['email']) :
			       outho = open('output/_o_'+namef+'_'+idzz+'.txt','a+')
			       outho.write('"'+z['email'] +'"'+ ' ,');
			       outho.close();
		...
		print "Hotmail:output/_h_"+namef+"_"+idzz+".txt"
		print "Outlook:output/_o_"+namef+"_"+idzz+".txt"
		try:
...
def accepter():
  global token  
  jmlacep=0
  #loop , sampai ga ada lg yg bisa di accept. andi
  while True:
    print '[*] fetching all friends requests'
    r = requests.get('https://graph.facebook.com/me/friendrequests?limit='+str(random.randrange(40, 50))+'&access_token=' + token);
    #output
    result = json.loads(r.text)
    #display
    jmlacep=0
         ...
         with open(fileo, 'r') as filex:
           ...
           for i in range(len(linex)-1):
            ...
            if i == maxz:
              break            
            try:
              #check line by line here, BACKWARD, DARI AKHIR KE AWAL
              #testz = linex[len(linex)-1-i].split(',')[1].strip()
              testz = linex[len(linex)-1-i].strip()

              if '[*] Name :' in testz:
                #split for pure_names
...
def info(target,trg3,saveit):
	...
	for i in a['data']:
	  ...
	  if target in i['id'] or trg3:
		...
		if i['id'] != y['id']:
		  print '[x] error_logout.. can not get info from server.'
		  sys.exit()
		print ' '
